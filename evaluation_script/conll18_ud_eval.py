...
# Command line usage
# ------------------
# conll18_ud_eval.py [-v] [-w weights_file] gold_conllu_file system_conllu_file
#
# - if no -v is given, only the CoNLL18 UD Shared Task evaluation LAS metrics
#   are printed
...
def load_conllu(file):
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            self.parent = None
            # Reference to the list of UDWord instances representing functional-deprel children
            self.func_children = []
            # Let's ignore language-specific deprel subtypes.
            self.columns[DEPREL] = columns[DEPREL].split(':')[0]
            # Let's ignore non-universal FEATS and sort the universal ones
            self.columns[FEATS] = '|'.join(x for x in sorted(columns[FEATS].split('|'))
                                           if x.split('=', 1)[0] in UNIV_FEATS)
...
def evaluate(gold_ud, system_ud, deprel_weights=None):
    ...
    def _morpho_match(alignment, words):
        w, g = words.system_word, words.gold_word
        if (w.columns[UPOS] != g.columns[UPOS] or
            w.columns[FEATS] != g.columns[FEATS] or
            len(w.func_children) != len(g.func_children)):
            return False
        for w_child, g_child in zip(w.func_children, g.func_children):
            if (alignment.matched_words_map.get(w_child) != g_child or
                w_child.columns[DEPREL] != g_child.columns[DEPREL] or
                w_child.columns[UPOS] != g_child.columns[UPOS] or
                w_child.columns[FEATS] != g_child.columns[FEATS]):
                return False
        return True
    ...
    # Initialize counts. "c_" stands for content words
    gold, system = len(gold_ud.words), len(system_ud.words)
    c_gold = len([w for w in gold_ud.words if w.columns[DEPREL] in CONTENT])
    c_system = len([w for w in system_ud.words if w.columns[DEPREL] in CONTENT])
    c_aligned, aligned, upos, xpos, feats, alltags, lemmas, uas, las, clas, blex, mlas = [0] * 12

    for words in alignment.matched_words:
        aligned += 1
        w, g = words.system_word, words.gold_word
        if g.columns[DEPREL] in CONTENT:
            c_aligned += 1
        if w.columns[UPOS] == g.columns[UPOS]:
            upos += 1
        if w.columns[XPOS] == g.columns[XPOS]:
            xpos += 1
        if w.columns[FEATS] == g.columns[FEATS]:
            feats += 1
        ...
        if words.gold_parent == words.system_parent_gold_aligned:
            uas += 1
            if w.columns[DEPREL] == g.columns[DEPREL]:
                las += 1
                if w.columns[DEPREL] in CONTENT:
                    clas += 1
                    if g.columns[LEMMA] == '_' or w.columns[LEMMA] == g.columns[LEMMA]:
                        blex += 1
                    if _morpho_match(alignment, words):
                        mlas += 1
    ...
    return {
        ...
        "LAS": Score(gold, system, las, aligned),
        "CLAS": Score(c_gold, c_system, clas, c_aligned),
        "BLEX": Score(c_gold, c_system, blex, c_aligned),
        "MLAS": Score(c_gold, c_system, mlas, c_aligned),
    }
